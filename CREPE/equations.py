...
## Some part of the code will be adjusted
## Equations
def e_max(td):
...
def pulsar(en, gamma, e_cut, eta, edot, td, delta, d0, e0, d):
    model = (Q0(gamma, e_cut, eta, edot, td) / (rdiff(en, delta, d0, e0, td)**3) )* ((1 - ((en *u.GeV)/e_max(td)))**(gamma-2)) * ((en *u.GeV)) * np.exp(-(en/ ((1- (en/e_max(td).value)) *e_cut))) * np.exp(- ((d*u.kpc).to(u.meter)).value**2/ (rdiff(en, delta, d0, e0, td).value)**2 ) * (c/(4*np.pi))*(u.sr**-1) * np.heaviside([e_max(td).value - en], 1)[0] *(en *u.GeV)**3
