...
file.write(f'PSR \t Edot (erg/s) \t Age (year) \t d (kpc) \t gamma \t E_cut \t eta \t etaEdot \t E_max (GeV) \t ReducedChi^2_Middle \t ReducedChi2_po_Middle \t reducedchi_2_combine_Old\n')
####

for i in np.arange(91, len(ATNF_NamePSR_Cut_Milli), 1):
        k = i
        ...
        file_name_CornerPlots_Old_PWN = f'Cornerplot_{[i]}_{ATNF_NamePSR_Cut_Milli[i]}.png'
        save_path_CornerPlots_Old_PWN = os.path.join(save_dir +'/CornerPlots/Old_PWN', file_name_CornerPlots_Old_PWN)
        ###
        file_name_WalkerSteps_Old_PWN = f'Walkerstep_{[i]}_{ATNF_NamePSR_Cut_Milli[i]}.png'
        save_path_WalkerSteps_Old_PWN = os.path.join(save_dir +'/WalkerSteps/Old_PWN', file_name_WalkerSteps_Old_PWN)
        ###
        file_name_Plotting_Old_PWN = f'[{i}]_{PSR}_loglogplot_parameters.png'
        save_path_Plotting_Old_PWN = os.path.join(save_dir + '/Plotting/Old_PWN', file_name_Plotting_Old_PWN)
        ###
        file_name_Plotting_Old_PWN2 = f'[{i}]_{PSR}_loglogplot_parameters_positrons.png'
        save_path_Plotting_Old_PWN2 = os.path.join(save_dir + '/Plotting_Positrons/Old_PWN', file_name_Plotting_Old_PWN2)
        ###
        file_name_Plotting_Old_PWN3 = f'[{i}]_{PSR}_loglogplot_parameters_Combined.png'
        save_path_Plotting_Old_PWN3 = os.path.join(save_dir + '/Plotting_Combined/Old_PWN', file_name_Plotting_Old_PWN3)
        ###

        print('PSR = ', PSR, 'Age = ', td, 'Dist = ', d, 'Edot = ', edot)

##########
        print("Start Running MCMC for Old PWN Case -- Find a drink while waiting")
        ...
        print('--------------------')
        theta_max = samples[np.argmax(sampler.flatlnprobability)]
        reducedchi_2_Old = round(reducedchi2(theta_max), 4)
        reducedchi_2po_Old = round(reducedchi2po(theta_max), 4)
        reducedchi_2_combine_Old = round(reducedchi2_combine(theta_max), 4)
        print('reducedchi_2 = ', reducedchi_2_Old)
        print('reducedchi_2po = ', reducedchi_2po_Old)
	print('reducedchi_2_combine_Old = ', reducedchi_2_combine_Old)
        ##### Write down parameters Fitting in dat file
        file.write(f'{ATNF_NamePSR_Cut_Milli[i]} \t {ATNF_Edot_Cut_Milli[i]} \t {ATNF_Age_Cut_Milli[i]} \t {ATNF_Dist_Cut_Milli[i]} \t {gamma:.2f} \t {e_cut:.2f} \t {eta:.2f} \t {eta*edot} \t {e_max(td).value:.2f} \t {reducedchi_2_Old:.2f} \t {reducedchi_2po_Old:.2f} \t {reducedchi_2_combine_Old:.2f}\n')
        file.flush()
        ...
        plt.savefig(f'{save_path_CornerPlots_Old_PWN}', dpi=300)
        plt.close()
        ### Walker Steps
        ...
        for i in range(ndim):
            ...
            plt.savefig(save_path_WalkerSteps_Old_PWN, dpi=300)
        plt.close()
        ...
        legend_title = rf"$\gamma$ = {gamma:.2f} $E_{{cut}}$ = {e_cut:.2f} $\eta$ = {eta:.2f} $\chi^2$ = {reducedchi_2_Old:.2f}"
        ...
        plt.savefig(save_path_Plotting_Old_PWN, dpi=300)
        ...
        legend_title = rf"$\gamma$ = {gamma:.2f} $E_{{cut}}$ = {e_cut:.2f} $\eta$ = {eta:.2f} $\chi^2$ = {reducedchi_2po_Old:.2f}"
        ...
        plt.yticks(fontsize=18)
        #### Save Figure in Folder
        plt.savefig(save_path_Plotting_Old_PWN2, dpi=300)
        plt.close()
        ...
        legend_title2 = rf"$\gamma$ = {gamma:.2f} $E_{{cut}}$ = {e_cut:.2f} $\eta$ = {eta:.2f} $\chi^2$ = {reducedchi_2_combine_Old:.2f}"
        ...
        plt.yticks(fontsize=18)
        #### Save Figure in Folder
        plt.savefig(save_path_Plotting_Old_PWN3, dpi=300)
